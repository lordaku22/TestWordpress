...
class FooEvents_Calendar {
    ...
    public function __construct() {
        ...
        add_action('admin_init', array($this, 'display_meta_errors'));      
    ...
    public function save_posts_meta_box($post_id) {
        ...
        if(isset($_POST['WooCommerceEventsType'])) {
            ...
            $WooCommerceEventsType = sanitize_text_field($_POST['WooCommerceEventsType']);
            update_post_meta($post_id, 'WooCommerceEventsType', $WooCommerceEventsType);
    ...
    public function process_eventbrite($post_id) {
        ...
        $event_date = '';
        if(isset($_POST['WooCommerceEventsMultiDayType']) && $_POST['WooCommerceEventsMultiDayType'] == 'select') {
        ...
        $event_date = $event_date.' '.$_POST['WooCommerceEventsHour'].':'.$_POST['WooCommerceEventsMinutes'].$_POST['WooCommerceEventsPeriod'];
    ...
    public function import_events_from_eventbrite() {
        ...
        $events = $client->get_user_owned_events($user['id']);
    ...
    public function events_list($attributes) {
        ...
        if (is_plugin_active('fooevents_bookings/fooevents-bookings.php') || is_plugin_active_for_network('fooevents_bookings/fooevents-bookings.php')) {
            ...
            $booking_events = $FooEvents_Bookings->get_bookings_for_calendar();
    ...
    public function fetch_events($events, $display_type, $include_desc = true) {
        ...
        foreach($events as $event) {
            ...
            $globalWooCommerceEventsExpireOption = get_option('globalWooCommerceEventsExpireOption');
            $today = time();
            
            //Check if event has expired
            if($globalWooCommerceEventsExpireOption == 'hide' && !empty($event_expire) && $today >= $event_expire) {
            ...
            $WooCommerceEventsEvent = get_post_meta($event->ID, 'WooCommerceEventsEvent', true);
            $today = time();
            
            if(!empty($timestamp) && $WooCommerceEventsEvent == 'Event' && $today > $timestamp) {
